...
//   const k = Math.sqrt(
//     Math.pow(featureDistance, 2) / (speedX * speedX + speedY * speedY)
//   );
//   const featurePath = {
//     dictance: featureDistance,
//     finalX: x + k * speedX,
//     finalY: y + k * speedY,
//   };
...
export function get(canvas: HTMLCanvasElement): IGet {
  ...
  class Particle implements IParticle {
    ...
    constructor(padding: number) {
      ...
      const cosAngle =
        this.speedX /
        Math.sqrt(Math.pow(this.speedX, 2) + Math.pow(this.speedY, 2));
      const sinAngle = Math.sqrt(1 - Math.pow(cosAngle, 2));
      this.featurePath = {
        dictance: this.allowedRadius,
        finalX: this.x + this.allowedRadius * cosAngle,
        finalY: this.y + this.allowedRadius * sinAngle,
      };
    }
    update(mouse: IMouse) {
      if (mouse.x && mouse.y) {
  ...
  class Line implements ILine {
    ...
    update(key: string, x: number, y: number, mouse: IMouse): void {
      if (mouse.x && mouse.y) {
        const isInMouseArea =
          Math.sqrt(
            Math.pow(mouse.x - this.x1, 2) + Math.pow(mouse.y - this.y1, 2)
          ) < mouse.radius;
        if (isInMouseArea) {
          this.color = mouseColor;
        } else {
